    ...
    <div className="bg-slate-900/85 backdrop-blur-sm rounded-lg border border-slate-700/40 overflow-hidden min-w-[160px]">
      ...
      <div className="p-2 flex flex-col gap-px">
        {BODIES.map(({ target, label, color }) => {
          ...
          return (
            <div key={target} className="flex items-stretch gap-0.5">
              <button
                onClick={() => onJumpTo(target)}
                className="flex-1 flex items-center gap-2.5 px-2.5 py-1 rounded text-xs text-slate-300 hover:text-white hover:bg-slate-700/60 transition-colors text-left"
              >
              ...
              <button
                ...
                className={`w-8 rounded text-xs font-medium transition-colors ${
                  isAnchored
        ...
        <button
          ...
          className="w-full px-2.5 py-1 rounded text-xs text-slate-400 hover:text-slate-200 hover:bg-slate-700/60 transition-colors text-center"
        >
          Full View
        </button>
      </div>
